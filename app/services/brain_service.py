...
class BrainService:
    ...
    async def process_chat(self, session_id: str, user_message: str, use_search: bool, is_voice: bool = False) -> AsyncGenerator[str, None]:
        ...
        # V5 Phase 4 Safeguard: Force COMMAND intent for critical Desktop Subsystems
        desktop_keywords = [
            "open ", "launch ", "start ", "move mouse", "click", "type ", "press ", 
            "hotkey", "system status", "running programs", "running processes", 
            "cpu usage", "ram usage", "installed tools"
        ]
        if any(kw in user_message.lower() for kw in desktop_keywords):
            intent = "COMMAND"
